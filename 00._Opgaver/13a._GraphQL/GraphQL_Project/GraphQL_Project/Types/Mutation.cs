...
namespace GraphQL_Project.Types
{
    ...
    public static class Mutation
    {
        ...
        public static async Task<Book?> CreateBookAsync([ID] Guid authorId, string title, int? releaseYear, [Service] IBookRepository repository, [Service] ITopicEventSender eventSender)
        {
        ...
        public static async Task<Book?> UpdateBookAsync([ID] Guid id, [ID] Guid? authorId, string? title, int? releaseYear, [Service] IBookRepository repository)
        {
            return await repository.UpdateBookAsync(id, authorId, title, releaseYear);
        }

        [GraphQLDescription("Delete a book by id")]
        public static async Task<SuccessMessage?> DeleteBookAsync([ID] Guid id, [Service] IBookRepository repository)
        {
